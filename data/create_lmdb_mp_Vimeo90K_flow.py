...
img_folder = '/home/lpc/dataset/natural_img/vimeo_triplet/flows/'
lmdb_save_path = '/home/lpc/dataset/natural_img/vimeo7_' + phase + '_flow.lmdb'
H_dst, W_dst = 256, 448
frame_number = 1
txt_file = '/home/lpc/dataset/natural_img/vimeo_triplet/tri_' + phase + 'list.txt'
...
def vimeo7():
    ...
    keys = sorted(keys)
    if mode == 'GT': 
        all_img_list = [v for v in all_img_list if v.endswith('.npy')]
    ...
    for path, key in zip(all_img_list, keys):
        ...
        img = np.load(path)   ## 读取数据
        img = np.ascontiguousarray(img.transpose(1, 2, 0))
        key_byte = key.encode('ascii')
        H, W, C = img.shape  # fixed shape
        # print(img.shape)
        assert H == H_dst and W == W_dst and C == 4, 'different shape.'
