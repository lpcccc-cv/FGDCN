...
class DCN_sep(DCNv2):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1,
        ...
        channels_ = self.deformable_groups * 3 * self.kernel_size[0] * self.kernel_size[1]
        self.conv_offset_mask = nn.Conv2d(64, channels_, kernel_size=self.kernel_size,
...
class DCN_sep_our(DCNv2):
    '''Use other features to generate offsets and masks'''

    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1,
                 deformable_groups=1):
        super(DCN_sep_our, self).__init__(in_channels, out_channels, kernel_size, stride, padding,
                                      dilation, deformable_groups)
    ...
    def forward(self, input, fea, flow):
        ...
        res_offset = offset
        n, c, h, w = offset.size()
        flow = flow.flip(1).repeat(1, c // 2, 1, 1)
        offset = offset+flow
        # offset = flow
