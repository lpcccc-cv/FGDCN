...
import models.modules.OpticalFlow.correlation_lite as correlation
# try:
...
def backwarp(tenInput, tenFlow):
    ...
    return torch.nn.functional.grid_sample(input=tenInput, grid=(backwarp_tenGrid[str(tenFlow.shape)].to(tenFlow.device) + tenFlow).permute(0, 2, 3, 1), mode='bilinear', padding_mode='zeros', align_corners=False)
# end
...
class Network(torch.nn.Module):
    ...
    def forward(self, tenOne, tenTwo):
        ...
        tenOne = tenPreprocessedOne
        tenTwo = tenPreprocessedTwo
...
if __name__ == '__main__':
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('../images/one.png'))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))).unsqueeze(0).cuda()
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('../images/two.png'))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))).unsqueeze(0).cuda()

    liteflow = Network().cuda()
    tenOutput = liteflow(tenOne, tenTwo)
    print(tenOutput.shape)
    flow1 = tenOutput.data.cpu()[0]
    flow1 = flow1.numpy().transpose((1,2,0))
    flow_im1 = flow_to_image(flow1)
    cv2.imwrite('./flow1_liteflow.png', flow_im1)
