        ...
        for _, train_data in enumerate(train_loader): 
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for i, val_data in enumerate(tqdm(val_loader)):               
                    ...
                    sr_img = visuals["restore"] 
                    gt_img = visuals["GT"]

                    # vis_img = util.tensor2img(sr_img)
                    # save_path = os.path.join(opt['path']['val_images'], '{:08d}.png'.format(current_step))
                    # cv2.imwrite(save_path, vis_img)

                    for num_img in range(sr_img.shape[0]):
                        # calculate PSNR
                        cur_psnr = util.calculate_psnr(sr_img[num_img, :, :, :].numpy()*255., gt_img[num_img, 1, :, :, :].numpy()*255.)
                        avg_psnr += cur_psnr
                        idx += 1    
